...
namespace ProjektProgramsko
{
	...
	public partial class WidgetPocetna : Gtk.Bin
	{
		public WidgetPocetna()
		{
			...
			//Casopisi
			if (listaI.Capacity != 0)
			{
				Casopis c = BPCasopis.DohvatiCasopis(listaI[0].IdC);
				var buffer = System.IO.File.ReadAllBytes(listaI[0].SlikaPath);
				var pixbuf = new Gdk.Pixbuf(buffer);
			...
			//Filmovi
			if (listaF.Capacity != 0)
			{
				var buffer = System.IO.File.ReadAllBytes(listaF[0].SlikaPath);
				var pixbuf = new Gdk.Pixbuf(buffer);
